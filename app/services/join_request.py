...
from repositories import JoinRequestRepository
...
class JoinRequestService(BaseService[JoinRequest, JoinRequestRepository]):
    ...
    async def create_jr(self, user_id: int, channel_id: int):
        """Создается при попытке подписки на приватный канал"""

        if jr := await self.repository.get_one_by(user_id=user_id, channel_id=channel_id):
            await self.delete_item_by_id(jr.id)

        await self.create_item(user_id=user_id, channel_id=channel_id)
